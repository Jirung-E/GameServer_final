...
int main()
{
	...
	// server 지정
	std::cout << "Enter server address(port: 4000)" << std::endl;
	std::cout << "ip(default: 127.0.0.1) > ";
	std::string server_ip;
	std::getline(std::cin, server_ip);
	if(server_ip.empty() || server_ip == "default") {
		server_ip = "127.0.0.1";
	}
	std::cout << "Connecting to server (" << server_ip << ":" << GAME_PORT << ")" << std::endl;

	sf::Socket::Status status = s_socket.connect(server_ip, GAME_PORT);
	...
	while(true) {
		...
		if(id.length() > MAX_ID_LENGTH) {
			std::cout << "ID is too long. Max length is " << MAX_ID_LENGTH << "." << std::endl;
			continue;
		}
		break;
	}
	...
	strcpy_s(p.name, id.c_str());
	send_packet(&p);
	avatar.set_name(p.name);

	char buf[BUF_SIZE];
	size_t received;
	auto recv_result = s_socket.receive(buf, BUF_SIZE, received);
	if(recv_result == sf::Socket::Error) {
		wcout << L"Recv 에러!";
		exit(-1);
	}
	...
	if(recv_result != sf::Socket::NotReady) {
		if(received > 0) process_data(buf, received);
	}
